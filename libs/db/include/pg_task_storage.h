...
public:
    ...
    Result<void> commitPlanChange(const StateChange& state, const PlanId planId) noexcept override;
    Result<std::vector<AllocationPendingVmInfo>> getVmsToAllocate(const size_t maxVmAllocationCount) noexcept override;
    ...
    Result<void> returnUnterminatedVms(const std::vector<VmId>& vmIds) noexcept override;
    Result<void> saveVmAllocationResult(const VmId id, const AllocatedVmInfo& allocatedVmInfo) noexcept override;
    ...
    Result<void> restartStaleAllocatingVms(
        const std::chrono::seconds& allocationTimeLimit, const size_t vmRestartAttemptCount) noexcept override;
    ...
    Result<void> terminateVmsWithInactiveAgents(
        const std::chrono::seconds& agentInactivityTimeLimit, const size_t jobRestartAttemptCount) noexcept override;
    Result<void> terminateVmsWithoutAgents(
        const std::chrono::seconds& agentStartupTimeLimit, const size_t jobRestartAttemptCount) noexcept override;
...
private:
    ...
    Result<void> refreshPlanActivityTs_(pqxx::transaction_base& txn, const PlanId currentPlanId) noexcept;
    ...
    Result<PlanId> insertNewPlanRecord_(pqxx::transaction_base& txn, const BackendId& backendId) noexcept;
    Result<void> terminateVms_(pqxx::transaction_base& txn, const std::vector<VmId>& vmsToTerminate) noexcept;
    Result<void> updateVmCapacities_(
        pqxx::transaction_base& txn, const std::vector<VmCapacityUpdate>& vmCapacityUpdates) noexcept;
    Result<void> applyJobVmAssignments_(pqxx::transaction_base& txn, const JobToVm& vmAssignments) noexcept;
