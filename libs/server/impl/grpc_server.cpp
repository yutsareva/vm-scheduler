...
GrpcServer::GrpcServer(const ServerConfig& config, TaskStorage* taskStorage)
    ...
    , publicApiSchedulerService_(*this)
    , agentApiSchedulerService_(*this)
{
    grpc::ServerBuilder builder;
    builder.AddListeningPort(config.address, grpc::InsecureServerCredentials());
    server_ = builder.RegisterService(&publicApiSchedulerService_)
                  .RegisterService(&agentApiSchedulerService_)
                  .BuildAndStart();
    if (!server_) {
        throw std::runtime_error(
            toString("Failed to start gRPC server on ", config.address));
    }
