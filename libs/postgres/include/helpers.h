...
std::string asFormattedList(const TContainer& items, const pqxx::transaction_base& txn)
{
    using TItem = typename TContainer::value_type;
    const auto format =
        [&txn](const TItem& item, std::stringstream& stream)
...
template<typename TContainer>
requires std::forward_iterator<typename TContainer::iterator> && IsEnum<typename TContainer::value_type>
...
std::vector<T> extractIds(const pqxx::result& pgResult)
{
    ...
    std::transform(pgResult.begin(), pgResult.end(), std::back_inserter(ids),
                   [](const pqxx::row row) -> T { return row.at("id").as<T>(); });
    return ids;
}

} // namespace maps::b2bgeo::pg
